...
(function () {
    ...
    const NotificationBell = {
         ...
         */
        init: function () {
            // Get API settings from localized script data
            // Get API settings from localized script data
            if (typeof pillpalnowNotifications !== 'undefined') {
                this.apiUrl = pillpalnowNotifications.apiUrl;
                this.nonce = pillpalnowNotifications.nonce;
                console.log('NotificationBell initialized with config:', pillpalnowNotifications);
            } else {
                console.warn('pillpalnowNotifications configuration missing');
            }
        ...
        loadUnreadCount: function () {
            // Always bypass browser cache for unread count
            fetch(this.apiUrl + 'notifications/unread-count?t=' + new Date().getTime(), {
                headers: {
        ...
        loadNotifications: function () {
            ...
            fetch(this.apiUrl + 'notifications?status=unread&limit=20', {
                headers: {
                    'X-WP-Nonce': this.nonce
                }
            })
        ...
        handleDeleteClick: function (e) {
            ...
            fetch(this.apiUrl + 'notifications/delete', {
                method: 'POST',
                headers: {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        // Refresh count from server directly (as backup)
                        this.loadUnreadCount();
                    } else {
                        // Revert
        ...
        markAsRead: function (notificationIds, callback) {
            fetch(this.apiUrl + 'notifications/mark-read', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-WP-Nonce': this.nonce
    ...
    // Expose for debugging and inline scripts
