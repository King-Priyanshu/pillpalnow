...
add_action('after_setup_theme', 'pillpalnow_setup');

/**
 * Enqueue scripts and styles.
 */
function pillpalnow_scripts()
{
	...
	// Only load notification bell for logged-in users (REST API requires auth)
	...
	wp_localize_script('pillpalnow-api', 'pillpalnow_vars', $pillpalnow_vars); // Localize API helper
	...
	// Localize notification script with API settings (only if enqueued)
	if (is_user_logged_in()) {
...
add_action('wp_enqueue_scripts', 'pillpalnow_scripts');

/**
 * Register App Menu Widget Area
 */
function pillpalnow_widgets_init()
{
...
add_action('widgets_init', 'pillpalnow_widgets_init');


/**
 * Redirect to Custom Login Page after Logout
 */
add_filter('logout_redirect', 'pillpalnow_logout_redirect', 10, 3);
function pillpalnow_logout_redirect($redirect_to, $requested_redirect_to, $user)
{
 ...
 */
function pillpalnow_global_auth_redirect()
{
...
add_action('template_redirect', 'pillpalnow_global_auth_redirect');
...
add_filter('pillpalnow_allowed_pages', function ($pages) {
	$pages[] = 'magic-login';
	return $pages;
});
 ...
 */
function pillpalnow_detect_webview_body_class($classes)
{
	$is_webview = false;
	...
	/* if (isset($_COOKIE['pillpalnow_webview']) && $_COOKIE['pillpalnow_webview'] === '1') {
		$is_webview = true;
	} */
...
add_filter('body_class', 'pillpalnow_detect_webview_body_class');

/**
 * Temporary Flush Rewrite Rules
 * Fixes 404 on family member pages. Remove after verification.
 */
// Enqueue refill actions script
function pillpalnow_enqueue_refill_scripts() {
    if (is_page('refills')) {
        wp_enqueue_script('pillpalnow-refill-actions', get_template_directory_uri() . '/assets/js/refill-actions.js', array('jquery', 'pillpalnow-api-helper'), '1.0.0', true);
    }
}
add_action('wp_enqueue_scripts', 'pillpalnow_enqueue_refill_scripts');
// Enqueue dynamic refill buttons script
function pillpalnow_enqueue_dynamic_refill_buttons() {
    if (is_page('refills')) {
        wp_enqueue_script('pillpalnow-dynamic-refill-buttons', get_template_directory_uri() . '/assets/js/dynamic-refill-buttons.js', array('jquery', 'pillpalnow-refill-actions'), '1.0.0', true);
    }
}
add_action('wp_enqueue_scripts', 'pillpalnow_enqueue_dynamic_refill_buttons');
